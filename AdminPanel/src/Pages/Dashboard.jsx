...
const Dashboard = () => {
  ...
  if (students.isLoading || leaves.isLoading || feedbacks.isLoading || complaints.isLoading) {
    return <Spinner message={'Getting Data.....'} />;
  }

  const bgColor = useColorModeValue('lightMode.bg', 'darkMode.bg');

  const filteredLeaves = leaves.LeavesList?.data?.filter(leave => {
    ...
    >
      ...
      <Flex flexWrap="wrap" gap="4">
        ...
        >
          ...
          <StatNumber fontSize="2xl">{students.studentsList?.length}</StatNumber>
        </Stat>
        <Stat flex="1" p={4} borderRadius="md" boxShadow="md">
          <StatLabel fontSize="lg">Total Leaves</StatLabel>
          <StatNumber fontSize="2xl">{leaves.LeavesList?.data?.length}</StatNumber>
        </Stat>
        <Stat flex="1" p={4} borderRadius="md" boxShadow="md">
          <StatLabel fontSize="lg">Total Feedbacks</StatLabel>
          <StatNumber fontSize="2xl">{feedbacks.feedbacks?.length}</StatNumber>
        </Stat>
        <Stat flex="1" p={4} borderRadius="md" boxShadow="md">
          <StatLabel fontSize="lg">Total Complaints</StatLabel>
          <StatNumber fontSize="2xl">{complaints.complaints?.length}</StatNumber>
        </Stat>
      </Flex>
      ...
      <Flex flexWrap="wrap" gap="4">
        ...
        <DashboardSection
          ...
          data={filteredFeedbacks && filteredFeedbacks.slice(0, 5)}
